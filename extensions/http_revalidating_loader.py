...
def return_contents(response, url, callback, context, path):
    context.statsd_client.incr('original_image.status.' + str(response.code))

    if response.code == 304 and exists(path):
        callback(open(path, 'r').read())
    elif response.error:
...
def load_sync(context, url, callback, normalize_url_func):
    ...
    url = normalize_url_func(url)
    path = path_on_filesystem(context,url)
    req = tornado.httpclient.HTTPRequest(
        ...
        if_modified_since=getmtime(path) if exists(path) else None
    )

    client.fetch(req, callback=partial(return_contents, url=url, callback=callback, context=context, path = path))

def encode(string):
    return None if string is None else string.encode('ascii')

def path_on_filesystem(context,path):
